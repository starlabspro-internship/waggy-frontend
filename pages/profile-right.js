export default function renderRightContent() {
  ...
  const fetchPets = async () => {
    try {
      ...
      if (response.ok) {
        window.pets = fetchedPets.filter(
          (fetchedPet) => fetchedPet.userId === userIdNumber
        ); // Update global pets array
      ...
      petItem.innerHTML = `
           ...
           <div class="flex flex-col md:flex-row md:space-x-5">
              ...
              <img src="http://localhost:3000${
                pet.petPicture || '/assets/images/default-pet.jpg'
              }"
                alt="${pet.name}" class="w-16 h-16 rounded-full">

              <div>
                <p class="font-bold text-color -mb-1">${pet.name}</p>
                <p class="text-xs text-red-500 -mb-1">${pet.breed} ${
        pet.species
      }</p>
              </div>
           </div>
